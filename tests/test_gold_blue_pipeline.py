...
class GoldBluePipelineTests(unittest.TestCase):
    ...
    def test_validation_notebooks_expose_mask_metrics_and_semantic_positions(self) -> None:
        ...
        self.assertIn('prior_kernel_state = {', sweep_code)
        self.assertIn('model = prior_kernel_state["model"]', sweep_code)
        self.assertIn('if adapter_name in loaded_peft_names:', sweep_code)
        self.assertIn('adapter_load_actions[word] = "reused"', sweep_code)
        self.assertIn('lens = prior_lens', sweep_code)
        self.assertIn('"Reusing Qwen 3.6 27B already loaded', sweep_code)
